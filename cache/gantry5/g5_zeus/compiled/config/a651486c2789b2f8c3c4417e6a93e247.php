<?php
return [
    '@class' => 'Gantry\\Component\\Config\\CompiledConfig',
    'timestamp' => 1764942797,
    'checksum' => '4fef95c77f3db921555e97a3877b8ca3',
    'files' => [
        'user/data/gantry5/themes/g5_zeus/config/home' => [
            'assignments' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/home/assignments.yaml',
                'modified' => 1764939829
            ],
            'index' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/home/index.yaml',
                'modified' => 1764939829
            ],
            'layout' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/home/layout.yaml',
                'modified' => 1764939829
            ],
            'styles' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/home/styles.yaml',
                'modified' => 1764939829
            ]
        ],
        'user/data/gantry5/themes/g5_zeus/config/default' => [
            'index' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/index.yaml',
                'modified' => 1764939829
            ],
            'layout' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/layout.yaml',
                'modified' => 1764939829
            ],
            'page/assets' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/page/assets.yaml',
                'modified' => 1764939829
            ],
            'page/body' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/page/body.yaml',
                'modified' => 1764939829
            ],
            'page/fontawesome' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/page/fontawesome.yaml',
                'modified' => 1764939829
            ],
            'page/head' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/page/head.yaml',
                'modified' => 1764939829
            ],
            'particles/bloglayoutSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/bloglayoutSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/branding' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/branding.yaml',
                'modified' => 1764939829
            ],
            'particles/calltoactionSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/calltoactionSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/contactSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/contactSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/content' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/content.yaml',
                'modified' => 1764939829
            ],
            'particles/contentarray' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/contentarray.yaml',
                'modified' => 1764939829
            ],
            'particles/contentcubesSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/contentcubesSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/contentframeSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/contentframeSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/contenttabs' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/contenttabs.yaml',
                'modified' => 1764939829
            ],
            'particles/copyright' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/copyright.yaml',
                'modified' => 1764939829
            ],
            'particles/custom' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/custom.yaml',
                'modified' => 1764939829
            ],
            'particles/customcontentSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/customcontentSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/date' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/date.yaml',
                'modified' => 1764939829
            ],
            'particles/featuredarticlesSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/featuredarticlesSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/headlineSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/headlineSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/horizontalmenu' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/horizontalmenu.yaml',
                'modified' => 1764939829
            ],
            'particles/imageSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/imageSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/imagegallerySZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/imagegallerySZ.yaml',
                'modified' => 1764939829
            ],
            'particles/lineSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/lineSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/login' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/login.yaml',
                'modified' => 1764939829
            ],
            'particles/logo' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/logo.yaml',
                'modified' => 1764939829
            ],
            'particles/menu' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/menu.yaml',
                'modified' => 1764939829
            ],
            'particles/messages' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/messages.yaml',
                'modified' => 1764939829
            ],
            'particles/mobile-menu' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/mobile-menu.yaml',
                'modified' => 1764939829
            ],
            'particles/owlcarousel' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/owlcarousel.yaml',
                'modified' => 1764939829
            ],
            'particles/portfolioSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/portfolioSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/position' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/position.yaml',
                'modified' => 1764939829
            ],
            'particles/pricetableSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/pricetableSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/sample' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/sample.yaml',
                'modified' => 1764939829
            ],
            'particles/search' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/search.yaml',
                'modified' => 1764939829
            ],
            'particles/skillsbarSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/skillsbarSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/social' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/social.yaml',
                'modified' => 1764939829
            ],
            'particles/socialfloatingSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/socialfloatingSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/spacer' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/spacer.yaml',
                'modified' => 1764939829
            ],
            'particles/testimonialSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/testimonialSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/timelineSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/timelineSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/topheadlinesSZ' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/topheadlinesSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/totop' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/particles/totop.yaml',
                'modified' => 1764939829
            ],
            'styles' => [
                'file' => 'user/data/gantry5/themes/g5_zeus/config/default/styles.yaml',
                'modified' => 1764939829
            ]
        ],
        'user/themes/g5_zeus/config/default' => [
            'page/body' => [
                'file' => 'user/themes/g5_zeus/config/default/page/body.yaml',
                'modified' => 1764939829
            ],
            'particles/branding' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/branding.yaml',
                'modified' => 1764939829
            ],
            'particles/contactSZ' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/contactSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/copyright' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/copyright.yaml',
                'modified' => 1764939829
            ],
            'particles/logo' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/logo.yaml',
                'modified' => 1764939829
            ],
            'particles/social' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/social.yaml',
                'modified' => 1764939829
            ],
            'particles/socialfloatingSZ' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/socialfloatingSZ.yaml',
                'modified' => 1764939829
            ],
            'particles/totop' => [
                'file' => 'user/themes/g5_zeus/config/default/particles/totop.yaml',
                'modified' => 1764939829
            ]
        ]
    ],
    'data' => [
        'particles' => [
            'contactSZ' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'style' => 'vertical',
                'maintitle' => 'Θα μας βρείτε',
                'contactinfo' => [
                    0 => [
                        'icon' => 'fa fa-soccer-ball-o',
                        'label' => 'Παρασκευή:',
                        'text' => '18:00 - 19:30',
                        'link' => '',
                        'linktarget' => '_self',
                        'variations' => 'line-bottom',
                        'title' => 'Παρασκευή'
                    ],
                    1 => [
                        'icon' => 'fa fa-users',
                        'label' => ' Σάββατο: ',
                        'text' => '15:30 - 16:30 μάθημα',
                        'link' => '',
                        'linktarget' => '_self',
                        'variations' => '',
                        'title' => 'Σαββάτο'
                    ],
                    2 => [
                        'icon' => 'fa fa-soccer-ball-o',
                        'label' => ' Σάββατο:',
                        'text' => '16:30 - 17:30',
                        'link' => '',
                        'linktarget' => '_self',
                        'variations' => '',
                        'title' => 'Σάββατο'
                    ],
                    3 => [
                        'icon' => 'fa fa-soccer-ball-o',
                        'label' => ' Κυριακή:',
                        'text' => '11:00 - 12:00',
                        'link' => '',
                        'linktarget' => '_self',
                        'variations' => '',
                        'title' => 'Κυριακή'
                    ]
                ]
            ],
            'contentcubesSZ' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                '_tab_link' => [
                    'linktext' => '&rarr;',
                    'buttonclass' => '',
                    'buttontarget' => '_self'
                ],
                '_tab_image' => [
                    'imageposition' => 'left'
                ],
                '_tab_link1' => [
                    'linkstyle1' => 'primary'
                ],
                '_tab_link2' => [
                    'linkstyle2' => 'secondary'
                ],
                '_tab_link3' => [
                    'linkstyle3' => 'outline'
                ],
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'items' => [
                    
                ],
                'css' => [
                    'class' => ''
                ]
            ],
            'contenttabs' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'animation' => 'left',
                'class' => '',
                'title' => '',
                'items' => [
                    
                ]
            ],
            'copyright' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'date' => [
                    'start' => '2018',
                    'end' => 'now'
                ],
                'target' => '_blank',
                'owner' => 'threethemes',
                'link' => 'https://threethemes.com',
                'additional' => [
                    'text' => 'Developed by threethemes.com'
                ],
                'css' => [
                    'class' => ''
                ]
            ],
            'horizontalmenu' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'target' => '_blank',
                'class' => '',
                'items' => [
                    
                ]
            ],
            'imageSZ' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                '_tab_link' => [
                    'link' => true,
                    'target' => '_self'
                ],
                'image' => '',
                'svg' => '',
                'text' => '',
                'link' => '0',
                'url' => '',
                'target' => '_self',
                'paddingTop' => '',
                'paddingBottom' => '',
                'paddingLeft' => '',
                'paddingRight' => '',
                'marginTop' => '',
                'marginBottom' => '',
                'marginLeft' => '',
                'marginRight' => ''
            ],
            'logo' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'target' => '_self',
                'link' => '1',
                'url' => '',
                'image' => '',
                'svg' => '',
                'text' => 'ΙΝΑΑΠΦ',
                'class' => 'g-logo g-logo-zeus'
            ],
            'owlcarousel' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'nav' => 'disable',
                'dots' => 'enable',
                'autoplay' => 'disable',
                'imageOverlay' => 'enable',
                'class' => '',
                'title' => '',
                'autoplaySpeed' => '',
                'items' => [
                    
                ]
            ],
            'socialfloatingSZ' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '0',
                'target' => '_blank',
                'display' => 'both',
                'position' => 'left',
                'css' => [
                    'class' => ''
                ],
                'title' => '',
                'items' => [
                    0 => [
                        'icon' => 'fa fa-google fa-2x',
                        'text' => 'google',
                        'link' => '#',
                        'name' => 'google'
                    ],
                    1 => [
                        'icon' => 'fa fa-twitter fa-2x',
                        'text' => 'twitter',
                        'link' => '#',
                        'name' => 'twitter'
                    ],
                    2 => [
                        'icon' => 'fa fa-facebook-official fa-2x',
                        'text' => 'facebook',
                        'link' => '#',
                        'name' => 'facebook'
                    ],
                    3 => [
                        'icon' => 'fa fa-youtube fa-2x',
                        'text' => 'youtube',
                        'link' => '#',
                        'name' => 'youtube'
                    ],
                    4 => [
                        'icon' => 'fa fa-instagram fa-2x',
                        'text' => 'instagram',
                        'link' => '#',
                        'name' => 'instagram'
                    ],
                    5 => [
                        'icon' => 'fa fa-envelope fa-2x',
                        'text' => 'email',
                        'link' => 'mailto:#',
                        'name' => 'email'
                    ]
                ]
            ],
            'branding' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'content' => '',
                'css' => [
                    'class' => 'g-branding'
                ]
            ],
            'custom' => [
                'caching' => [
                    'type' => 'config_matches',
                    'values' => [
                        'twig' => '0',
                        'filter' => '0'
                    ]
                ],
                'enabled' => '1',
                'twig' => '0',
                'filter' => '0'
            ],
            'menu' => [
                'caching' => [
                    'type' => 'menu'
                ],
                'enabled' => '1',
                'menu' => '',
                'base' => '/',
                'startLevel' => '1',
                'maxLevels' => '0',
                'renderTitles' => '0',
                'hoverExpand' => '1',
                'mobileTarget' => '0',
                'forceTarget' => '0'
            ],
            'mobile-menu' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1'
            ],
            'social' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '0',
                'css' => [
                    'class' => ''
                ],
                'target' => '_blank',
                'display' => 'both',
                'title' => '',
                'items' => [
                    0 => [
                        'icon' => 'fa fa-facebook',
                        'text' => '',
                        'link' => '#',
                        'name' => 'facebook'
                    ],
                    1 => [
                        'icon' => 'fa fa-twitter',
                        'text' => '',
                        'link' => '#',
                        'name' => 'twitter'
                    ],
                    2 => [
                        'icon' => 'fa fa-google-plus',
                        'text' => '',
                        'link' => '#',
                        'name' => 'google+'
                    ],
                    3 => [
                        'icon' => 'fa fa-linkedin',
                        'text' => '',
                        'link' => '#',
                        'name' => 'linkendin'
                    ],
                    4 => [
                        'icon' => 'fa fa-envelope-o',
                        'text' => '',
                        'link' => 'mailto:',
                        'name' => 'email'
                    ]
                ]
            ],
            'spacer' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1'
            ],
            'totop' => [
                'caching' => [
                    'type' => 'static'
                ],
                'enabled' => '1',
                'css' => [
                    'class' => ''
                ],
                'icon' => 'fa fa-chevron-up fa-fw',
                'content' => 'Προς τα πάνω',
                'title' => ''
            ],
            'bloglayoutSZ' => [
                'enabled' => '1',
                'layout' => 'none',
                'article' => [
                    'filter' => [
                        'featured' => ''
                    ],
                    'limit' => [
                        'total' => 2,
                        'columns' => 2,
                        'start' => 0
                    ],
                    'sort' => [
                        'orderby' => 'publish_up',
                        'ordering' => 'ASC'
                    ],
                    'display' => [
                        'image' => [
                            'enabled' => 'intro'
                        ],
                        'text' => [
                            'type' => 'intro',
                            'limit' => '',
                            'formatting' => 'text'
                        ],
                        'title' => [
                            'enabled' => 'show',
                            'limit' => ''
                        ],
                        'date' => [
                            'enabled' => 'published',
                            'format' => 'l, F d, Y'
                        ],
                        'author' => [
                            'enabled' => 'show'
                        ],
                        'category' => [
                            'enabled' => 'link'
                        ],
                        'hits' => [
                            'enabled' => 'show'
                        ],
                        'pagination_buttons' => '',
                        'read_more' => [
                            'enabled' => 'show',
                            'label' => '',
                            'css' => ''
                        ]
                    ]
                ],
                'imgposition' => 'Left',
                'imgsize' => 'Small'
            ],
            'calltoactionSZ' => [
                'enabled' => '1',
                '_tab_link1' => [
                    'linkstyle1' => 'primary'
                ],
                'linkstyle2' => '',
                'linkstyle3' => '',
                'link1' => '',
                'linktext1' => '',
                'linkstyle1' => '',
                'link2' => '',
                'linktext2' => '',
                'link3' => '',
                'linktext3' => ''
            ],
            'contentframeSZ' => [
                'enabled' => '1',
                'image' => '',
                'mainicon' => '',
                'maintitle' => '',
                'subtitle' => '',
                'description' => ''
            ],
            'customcontentSZ' => [
                'enabled' => '1',
                '_tab_main' => [
                    'textalign' => ''
                ],
                '_tab_link1' => [
                    'linkstyle' => 'primary'
                ],
                '_tab_link2' => [
                    'linkstyle2' => 'secondary'
                ],
                '_tab_link3' => [
                    'linkstyle3' => 'outline'
                ],
                'height' => '',
                'image' => '',
                'mainicon' => '',
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'textalign' => '',
                'link' => '',
                'linktext' => '',
                'linkstyle' => '',
                'link2' => '',
                'linktext2' => '',
                'linkstyle2' => '',
                'link3' => '',
                'linktext3' => '',
                'linkstyle3' => '',
                'subitems' => [
                    
                ]
            ],
            'featuredarticlesSZ' => [
                'enabled' => '1',
                'layout' => 'Horizontal',
                'gutter' => 'No',
                'overlay' => 'No',
                'article' => [
                    'filter' => [
                        'featured' => 'include',
                        'categories' => '',
                        'articles' => ''
                    ],
                    'limit' => [
                        'total' => '5',
                        'start' => '0'
                    ],
                    'sort' => [
                        'orderby' => 'publish_up',
                        'ordering' => 'ASC'
                    ],
                    'display' => [
                        'image' => [
                            'enabled' => 'intro'
                        ],
                        'text' => [
                            'type' => 'intro',
                            'limit' => '',
                            'formatting' => 'text'
                        ],
                        'category' => [
                            'enabled' => 'link'
                        ],
                        'date' => [
                            'enabled' => 'published',
                            'format' => 'l, F d, Y'
                        ],
                        'author' => [
                            'enabled' => 'show'
                        ],
                        'title' => [
                            'limit' => ''
                        ]
                    ]
                ],
                'height' => '',
                'css' => [
                    'class' => ''
                ],
                'extra' => [
                    
                ],
                'maintitle' => ''
            ],
            'headlineSZ' => [
                'enabled' => '1',
                'tag' => 'h2',
                'maintitlealignment' => 'left',
                'maintitle' => '',
                'maintitlesize' => '',
                'maintitlecolor' => ''
            ],
            'imagegallerySZ' => [
                'enabled' => '1',
                'galleryimages' => [
                    
                ]
            ],
            'lineSZ' => [
                'enabled' => '1',
                'alignment' => 'left',
                'width' => '',
                'height' => '',
                'color' => ''
            ],
            'portfolioSZ' => [
                'enabled' => '1',
                'columns' => 'grid3',
                'gutter' => 'No',
                'filterPortfolios' => 'No',
                'displayTags' => 'No',
                'displayImageOnly' => 'No',
                '_tab_link1' => [
                    'linkstyle1' => ''
                ],
                '_tab_link2' => [
                    'linkstyle2' => 'secondary'
                ],
                '_tab_link3' => [
                    'linkstyle3' => 'outline'
                ],
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'portfolios' => [
                    
                ]
            ],
            'pricetableSZ' => [
                'enabled' => '1',
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'pricetables' => [
                    
                ]
            ],
            'sample' => [
                'enabled' => '1',
                'image' => '',
                'headline' => '',
                'description' => '',
                'link' => '',
                'linktext' => '',
                'samples' => [
                    
                ]
            ],
            'skillsbarSZ' => [
                'enabled' => '1',
                'style' => 'style1',
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'bars' => [
                    
                ]
            ],
            'testimonialSZ' => [
                'enabled' => '1',
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'testimonials' => [
                    
                ]
            ],
            'timelineSZ' => [
                'enabled' => '1',
                'maintitle' => '',
                'subtitle' => '',
                'description' => '',
                'events' => [
                    
                ]
            ],
            'topheadlinesSZ' => [
                'enabled' => '0',
                'layout' => 'default',
                'article' => [
                    'filter' => [
                        'featured' => ''
                    ],
                    'limit' => [
                        'total' => 2,
                        'columns' => 2,
                        'start' => 0
                    ],
                    'display' => [
                        'pagination_buttons' => '',
                        'image' => [
                            'enabled' => 'intro'
                        ],
                        'text' => [
                            'type' => 'intro',
                            'limit' => '',
                            'formatting' => 'text'
                        ],
                        'title' => [
                            'enabled' => 'show',
                            'limit' => ''
                        ],
                        'date' => [
                            'enabled' => 'published',
                            'format' => 'l, F d, Y'
                        ],
                        'read_more' => [
                            'enabled' => 'show',
                            'label' => '',
                            'css' => ''
                        ],
                        'author' => [
                            'enabled' => 'show'
                        ],
                        'category' => [
                            'enabled' => 'link'
                        ],
                        'hits' => [
                            'enabled' => 'show'
                        ]
                    ],
                    'sort' => [
                        'orderby' => 'publish_up',
                        'ordering' => 'ASC'
                    ]
                ],
                'css' => [
                    'class' => ''
                ],
                'extra' => [
                    
                ]
            ],
            'analytics' => [
                'enabled' => true,
                'ua' => [
                    'anonym' => false
                ]
            ],
            'assets' => [
                'enabled' => true
            ],
            'breadcrumbs' => [
                'enabled' => true
            ],
            'content' => [
                'enabled' => '1'
            ],
            'contentarray' => [
                'enabled' => '1',
                'article' => [
                    'limit' => [
                        'total' => 2,
                        'columns' => 2,
                        'start' => 0
                    ],
                    'display' => [
                        'pagination_buttons' => '',
                        'image' => [
                            'enabled' => 'intro'
                        ],
                        'text' => [
                            'type' => 'intro',
                            'limit' => '',
                            'formatting' => 'text'
                        ],
                        'title' => [
                            'enabled' => 'show',
                            'limit' => ''
                        ],
                        'date' => [
                            'enabled' => 'published',
                            'format' => 'l, F d, Y'
                        ],
                        'read_more' => [
                            'enabled' => 'show',
                            'label' => '',
                            'css' => ''
                        ],
                        'author' => [
                            'enabled' => 'show'
                        ],
                        'category' => [
                            'enabled' => 'show',
                            'route' => ''
                        ]
                    ],
                    'sort' => [
                        'orderby' => 'default',
                        'ordering' => 'asc'
                    ]
                ],
                'css' => [
                    'class' => ''
                ],
                'extra' => [
                    
                ]
            ],
            'date' => [
                'enabled' => '1',
                'css' => [
                    'class' => 'date'
                ],
                'date' => [
                    'formats' => 'l, F d, Y'
                ]
            ],
            'feed' => [
                'enabled' => true
            ],
            'frameworks' => [
                'enabled' => true,
                'jquery' => [
                    'enabled' => 0,
                    'ui_core' => 0,
                    'ui_sortable' => 0
                ],
                'bootstrap2' => [
                    'enabled' => 0
                ],
                'bootstrap3' => [
                    'enabled' => 0
                ],
                'bootstrap4' => [
                    'enabled' => 0
                ],
                'bootstrap5' => [
                    'enabled' => 0
                ],
                'mootools' => [
                    'enabled' => 0,
                    'more' => 0
                ]
            ],
            'langswitcher' => [
                'enabled' => true
            ],
            'lightcase' => [
                'enabled' => true
            ],
            'login' => [
                'enabled' => '1'
            ],
            'messages' => [
                'enabled' => '1'
            ],
            'position' => [
                'enabled' => '1',
                'chrome' => ''
            ],
            'search' => [
                'enabled' => '1'
            ]
        ],
        'styles' => [
            'above' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'accent' => [
                'color-1' => '#e31c3d',
                'color-2' => '#9bdaf1',
                'color-3' => '#112e51'
            ],
            'base' => [
                'dominant-color' => '#0071bc',
                'text-color' => '#212121',
                'background' => '#f5f5f5',
                'content-margin' => '0.5rem',
                'content-padding' => '0.5rem',
                'background-image' => ''
            ],
            'breakpoints' => [
                'large-desktop-container' => '75rem',
                'desktop-container' => '60rem',
                'tablet-container' => '48rem',
                'large-mobile-container' => '30rem',
                'mobile-menu-breakpoint' => '48rem'
            ],
            'expanded' => [
                'section-preset' => 'accent2',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'features' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'font' => [
                'family-default' => 'family=Ubuntu:300',
                'family-title' => 'Lato, Helvetica, Tahoma, Geneva, Arial, sans-serif',
                'family-quote' => 'Lato, Helvetica, Tahoma, Geneva, Arial, sans-serif'
            ],
            'footer' => [
                'section-preset' => 'disabled',
                'text-color' => '#f5f5f5',
                'link-color' => '#9bdaf1',
                'background' => '#112e51'
            ],
            'footerbottom' => [
                'section-preset' => 'disabled',
                'text-color' => '#f5f5f5',
                'link-color' => '#9bdaf1',
                'background' => '#112e51'
            ],
            'footertop' => [
                'section-preset' => 'accent2',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'header' => [
                'section-preset' => 'disabled',
                'text-color' => '#dbdbdb',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'intro' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'mainbottom' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'menu' => [
                'col-width' => '180px',
                'animation' => 'g-fade'
            ],
            'nadir' => [
                'section-preset' => 'disabled',
                'text-color' => '#f5f5f5',
                'link-color' => '#0071bc',
                'background' => '#000000'
            ],
            'navigation' => [
                'text-color' => '#f5f5f5',
                'link-color' => '#f5f5f5',
                'background' => '#112e51'
            ],
            'navtop' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'offcanvas' => [
                'background' => '#0071bc',
                'text-color' => '#f5f5f5',
                'toggle-color' => '#e31c3d',
                'toggle-visibility' => '1',
                'width' => '12rem'
            ],
            'testimonials' => [
                'section-preset' => 'accent1',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'utility' => [
                'section-preset' => 'disabled',
                'text-color' => '#212121',
                'link-color' => '#0071bc',
                'background' => '#f5f5f5'
            ],
            'zenith' => [
                'section-preset' => 'disabled',
                'text-color' => '#f5f5f5',
                'link-color' => '#0071bc',
                'background' => '#000000'
            ],
            'preset' => 'preset1'
        ],
        'page' => [
            'body' => [
                'attribs' => [
                    'class' => 'gantry g-zeus-style',
                    'id' => '',
                    'extra' => [
                        
                    ]
                ],
                'layout' => [
                    'sections' => '0'
                ],
                'doctype' => 'html',
                'body_top' => '',
                'body_bottom' => ''
            ],
            'fontawesome' => [
                'enable' => '1',
                'version' => 'fa4',
                'fa4_compatibility' => 1,
                'content_compatibility' => 1
            ],
            'assets' => [
                'favicon' => '',
                'touchicon' => '',
                'css' => [
                    
                ],
                'javascript' => [
                    
                ]
            ],
            'head' => [
                'meta' => [
                    
                ],
                'head_bottom' => '<!-- Global site tag (gtag.js) - Google Analytics --><script async src="https://www.googletagmanager.com/gtag/js?id=G-5R83HRJ6HD"></script><script> window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag(\'js\', new Date()); gtag(\'config\', \'G-5R83HRJ6HD\');</script>',
                'atoms' => [
                    
                ]
            ]
        ],
        'pages' => [
            'blog_item' => [
                'header' => [
                    'process' => [
                        'markdown' => true,
                        'twig' => false
                    ],
                    'child_type' => 'default',
                    'admin' => [
                        'children_display_order' => 'collection'
                    ]
                ],
                'name' => 'default'
            ],
            'blog_list' => [
                'header' => [
                    'process' => [
                        'markdown' => true,
                        'twig' => false
                    ],
                    'child_type' => 'blog_item',
                    'admin' => [
                        'children_display_order' => 'collection'
                    ],
                    'content' => [
                        'items' => '@self.children',
                        'leading' => 0,
                        'columns' => 2,
                        'limit' => 5,
                        'order' => [
                            'by' => 'date',
                            'dir' => 'desc'
                        ],
                        'show_date' => 0,
                        'pagination' => 1,
                        'url_taxonomy_filters' => 1
                    ]
                ],
                'name' => 'default'
            ],
            'modular' => [
                'features' => [
                    'name' => 'modular/features',
                    'header' => [
                        'template' => 'modular/features'
                    ]
                ],
                'showcase' => [
                    'name' => 'modular/showcase',
                    'header' => [
                        'template' => 'modular/showcase',
                        'buttons' => [
                            'primary' => 1
                        ]
                    ]
                ],
                'text' => [
                    'name' => 'modular/text',
                    'header' => [
                        'template' => 'modular/text',
                        'image_align' => 'left'
                    ]
                ]
            ]
        ],
        'index' => [
            'name' => 'home',
            'timestamp' => 1727862185,
            'version' => 7,
            'preset' => [
                'image' => 'gantry-admin://layout/homepage1.png',
                'name' => 'personal_3',
                'timestamp' => 1544983203
            ],
            'positions' => [
                
            ],
            'sections' => [
                'zenith' => 'Zenith',
                'navtop' => 'Navtop',
                'navigation' => 'Navigation',
                'intro' => 'Intro',
                'features' => 'Features',
                'utility' => 'Utility',
                'above' => 'Above',
                'testimonials' => 'Testimonials',
                'expanded' => 'Expanded',
                'mainbottom' => 'Mainbottom',
                'footertop' => 'Footertop',
                'footerbottom' => 'Footerbottom',
                'nadir' => 'Nadir',
                'header' => 'Header',
                'aside' => 'Aside',
                'sidebar' => 'Sidebar',
                'mainbar' => 'Mainbar',
                'footer' => 'Footer',
                'offcanvas' => 'Offcanvas'
            ],
            'particles' => [
                'logo' => [
                    'logo-9501' => 'Logo / Image',
                    'logo-2735' => 'Logo / Image'
                ],
                'menu' => [
                    'menu-6609' => 'Menu'
                ],
                'customcontentSZ' => [
                    'customcontentSZ-5538' => 'Φύγαμε',
                    'customcontentSZ-7843' => 'Το κατηχητικό ξεκίνησε'
                ],
                'skillsbarSZ' => [
                    'skillsbarSZ-2274' => 'όλα στο μάξιμουμ'
                ],
                'headlineSZ' => [
                    'headlineSZ-9001' => 'Φωτογραφίες',
                    'headlineSZ-2194' => 'Δραστηριότητες',
                    'headlineSZ-5679' => 'Headline',
                    'headlineSZ-7059' => 'πρόγραμμα',
                    'headlineSZ-4897' => 'αγόρια',
                    'headlineSZ-4991' => 'Κορίτσια',
                    'headlineSZ-7611' => 'Θα μας βρείτε',
                    'headlineSZ-3311' => '1ο Γυμνάσιο',
                    'headlineSZ-4280' => 'διαδικτυακό μαθημα'
                ],
                'imageSZ' => [
                    'imageSZ-3679' => 'η ζωή στο κατηχητικό',
                    'imageSZ-1645' => 'δημιουργια',
                    'imageSZ-6197' => 'συναγωνισμός',
                    'imageSZ-7475' => 'εξορμήσεις',
                    'imageSZ-8910' => 'γιορτές',
                    'imageSZ-7464' => 'mathainoume',
                    'imageSZ-2140' => 'Naos',
                    'imageSZ-3116' => 'Image',
                    'imageSZ-7514' => 'katotero',
                    'imageSZ-6182' => 'meso',
                    'imageSZ-2932' => 'anotero'
                ],
                'owlcarousel' => [
                    'owlcarousel-2640' => 'Στιγμές δραστηριοτήτων'
                ],
                'mobile-menu' => [
                    'mobile-menu-5697' => 'Mobile-menu'
                ],
                'messages' => [
                    'system-messages-4092' => 'System Messages'
                ],
                'custom' => [
                    'custom-5474' => 'Μία ώρα την εβδομάδα'
                ],
                'contactSZ' => [
                    'contactSZ-7900' => 'Νηπιακό',
                    'contactSZ-1520' => 'Αγόρια - Κατώτερο',
                    'contactSZ-7614' => 'Αγόρια - Μέσο, Ανώτερο',
                    'contactSZ-7260' => 'Νηπιακό',
                    'contactSZ-2733' => 'Κορίτσια - Δημοτικό',
                    'contactSZ-3003' => 'Κορίτσια - Μέσο',
                    'contactSZ-6014' => 'Κορίτσια - Ανώτερο',
                    'contactSZ-6973' => 'Contact Info'
                ],
                'lineSZ' => [
                    'lineSZ-4885' => 'Line'
                ],
                'totop' => [
                    'totop-6595' => 'To Top'
                ],
                'socialfloatingSZ' => [
                    'socialfloatingSZ-9045' => 'Social Floating'
                ]
            ],
            'inherit' => [
                'default' => [
                    'zenith' => 'zenith',
                    'navtop' => 'navtop',
                    'expanded' => 'expanded',
                    'mainbottom' => 'mainbottom',
                    'footertop' => 'footertop',
                    'footer' => 'footer',
                    'footerbottom' => 'footerbottom',
                    'nadir' => 'nadir',
                    'system-messages-4092' => 'system-messages-5272',
                    'headlineSZ-5679' => 'headlineSZ-4932',
                    'headlineSZ-7059' => 'headlineSZ-6611',
                    'custom-5474' => 'custom-8164',
                    'headlineSZ-4897' => 'headlineSZ-9750',
                    'contactSZ-7900' => 'contactSZ-4529',
                    'contactSZ-1520' => 'contactSZ-4713',
                    'contactSZ-7614' => 'contactSZ-8783',
                    'lineSZ-4885' => 'lineSZ-1122',
                    'headlineSZ-4991' => 'headlineSZ-4616',
                    'contactSZ-7260' => 'contactSZ-9218',
                    'contactSZ-2733' => 'contactSZ-6677',
                    'contactSZ-3003' => 'contactSZ-3493',
                    'contactSZ-6014' => 'contactSZ-5404',
                    'headlineSZ-7611' => 'headlineSZ-6311',
                    'imageSZ-2140' => 'imageSZ-6562',
                    'headlineSZ-3311' => 'headlineSZ-6901',
                    'imageSZ-3116' => 'imageSZ-6517',
                    'headlineSZ-4280' => 'headlineSZ-1665',
                    'imageSZ-7514' => 'imageSZ-9177',
                    'imageSZ-6182' => 'imageSZ-1326',
                    'imageSZ-2932' => 'imageSZ-9695',
                    'logo-2735' => 'logo-9571',
                    'contactSZ-6973' => 'contactSZ-7720',
                    'totop-6595' => 'totop-8670',
                    'socialfloatingSZ-9045' => 'socialfloatingSZ-2700'
                ]
            ]
        ],
        'layout' => [
            'version' => 2,
            'preset' => [
                'image' => 'gantry-admin://layout/homepage1.png',
                'name' => 'personal_3',
                'timestamp' => 1544983203
            ],
            'layout' => [
                'zenith' => [
                    
                ],
                'navtop' => [
                    
                ],
                '/navigation/' => [
                    0 => [
                        0 => 'logo-9501 15',
                        1 => 'menu-6609 85'
                    ]
                ],
                '/header/' => [
                    0 => [
                        0 => 'customcontentSZ-5538'
                    ]
                ],
                '/intro/' => [
                    0 => [
                        0 => 'headlineSZ-9001'
                    ],
                    1 => [
                        0 => 'owlcarousel-2640'
                    ]
                ],
                '/features/' => [
                    0 => [
                        0 => 'skillsbarSZ-2274'
                    ]
                ],
                '/utility/' => [
                    0 => [
                        0 => 'headlineSZ-2194'
                    ],
                    1 => [
                        0 => 'imageSZ-3679'
                    ],
                    2 => [
                        0 => 'imageSZ-1645'
                    ],
                    3 => [
                        0 => 'imageSZ-6197'
                    ],
                    4 => [
                        0 => 'imageSZ-7475'
                    ],
                    5 => [
                        0 => 'imageSZ-8910'
                    ]
                ],
                '/above/' => [
                    0 => [
                        0 => 'imageSZ-7464'
                    ]
                ],
                '/testimonials/' => [
                    0 => [
                        0 => 'customcontentSZ-7843'
                    ]
                ],
                'expanded' => [
                    
                ],
                '/container-main/' => [
                    0 => [
                        0 => [
                            'aside 25' => [
                                
                            ]
                        ],
                        1 => [
                            'mainbar 50' => [
                                
                            ]
                        ],
                        2 => [
                            'sidebar 25' => [
                                
                            ]
                        ]
                    ]
                ],
                'mainbottom' => [
                    
                ],
                'footertop' => [
                    
                ],
                'footer' => [
                    
                ],
                'footerbottom' => [
                    
                ],
                'nadir' => [
                    
                ],
                'offcanvas' => [
                    0 => [
                        0 => 'mobile-menu-5697'
                    ]
                ]
            ],
            'structure' => [
                'zenith' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'navtop' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'navigation' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '3',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => '',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => '',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => '',
                        'blockHeight' => ''
                    ]
                ],
                'header' => [
                    'attributes' => [
                        'boxed' => '',
                        'class' => '',
                        'background' => 'gantry-media://fygame_gia_katixitiko_3.jpg',
                        'backgroundOverlay' => 'rgba(0, 0, 0, 0.35)',
                        'backgroundAttachment' => 'scroll',
                        'backgroundRepeat' => 'no-repeat',
                        'backgroundPosition' => 'center center',
                        'backgroundSize' => 'cover',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => '',
                        'blockHeight' => '80vh'
                    ]
                ],
                'intro' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '0',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => 'rgba(0, 0, 0, 0)',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => '',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => ''
                    ]
                ],
                'features' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '0',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => 'rgba(0, 0, 0, 0)',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => 'cover',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => ''
                    ]
                ],
                'utility' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '0',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => 'rgba(3, 3, 3, 0.52)',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => 'cover',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => '',
                        'blockHeight' => ''
                    ]
                ],
                'above' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '0',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => 'rgba(0, 0, 0, 0.67)',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => 'cover',
                        'paddingTop' => '',
                        'paddingBottom' => '',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => ''
                    ]
                ],
                'testimonials' => [
                    'type' => 'section',
                    'attributes' => [
                        'boxed' => '0',
                        'class' => '',
                        'background' => '',
                        'backgroundOverlay' => 'rgba(0, 0, 0, 0)',
                        'backgroundAttachment' => '',
                        'backgroundRepeat' => '',
                        'backgroundPosition' => '',
                        'backgroundSize' => 'cover',
                        'paddingTop' => '4rem',
                        'paddingBottom' => '3rem',
                        'paddingLeft' => '',
                        'paddingRight' => '',
                        'marginTop' => '',
                        'marginBottom' => '',
                        'marginLeft' => '',
                        'marginRight' => ''
                    ]
                ],
                'expanded' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'aside' => [
                    'attributes' => [
                        'class' => ''
                    ],
                    'block' => [
                        'fixed' => '1'
                    ]
                ],
                'mainbar' => [
                    'type' => 'section',
                    'subtype' => 'main'
                ],
                'sidebar' => [
                    'type' => 'section',
                    'subtype' => 'aside',
                    'attributes' => [
                        'class' => ''
                    ],
                    'block' => [
                        'fixed' => '1'
                    ]
                ],
                'container-main' => [
                    'attributes' => [
                        'boxed' => ''
                    ]
                ],
                'mainbottom' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'footertop' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'footer' => [
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'footerbottom' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'nadir' => [
                    'type' => 'section',
                    'inherit' => [
                        'outline' => 'default',
                        'include' => [
                            0 => 'attributes',
                            1 => 'children'
                        ]
                    ]
                ],
                'offcanvas' => [
                    'attributes' => [
                        'position' => 'g-offcanvas-left',
                        'class' => '',
                        'extra' => [
                            
                        ],
                        'swipe' => '1',
                        'css3animation' => '1'
                    ]
                ]
            ],
            'content' => [
                'logo-9501' => [
                    'title' => 'Logo / Image'
                ],
                'menu-6609' => [
                    'attributes' => [
                        'menu' => 'mainmenu'
                    ]
                ],
                'customcontentSZ-5538' => [
                    'title' => 'Φύγαμε',
                    'attributes' => [
                        'height' => '50vh',
                        'maintitle' => 'Φύγαμε για Κατηχητικό!',
                        'subtitle' => 'Στον Άγιο Αλέξανδρο',
                        'description' => ''
                    ],
                    'block' => [
                        'variations' => 'margin-top-30'
                    ]
                ],
                'skillsbarSZ-2274' => [
                    'title' => 'όλα στο μάξιμουμ',
                    'attributes' => [
                        'style' => 'style2',
                        'maintitle' => 'Όλα στο μάξιμουμ!',
                        'bars' => [
                            0 => [
                                'skillTitle' => 'Χαρά',
                                'percentance' => '100',
                                'color' => '#ff4580',
                                'title' => 'Χαρά'
                            ],
                            1 => [
                                'skillTitle' => 'Δημιουργικότητα',
                                'percentance' => '100',
                                'color' => '#edf513',
                                'title' => 'Δημιουργικότητα'
                            ],
                            2 => [
                                'skillTitle' => 'Παιχνίδι',
                                'percentance' => '100',
                                'color' => '#00bd55',
                                'title' => 'Παιχνίδι'
                            ],
                            3 => [
                                'skillTitle' => ' Φιλία',
                                'percentance' => '100',
                                'color' => '#0071bc',
                                'title' => 'Φιλία'
                            ],
                            4 => [
                                'skillTitle' => ' Γέλιο',
                                'percentance' => '100',
                                'color' => '#bd0000',
                                'title' => 'Γέλιο'
                            ],
                            5 => [
                                'skillTitle' => ' Ψυχαγωγία',
                                'percentance' => '100',
                                'color' => '#bd009e',
                                'title' => 'Ψυχαγωγία'
                            ]
                        ]
                    ]
                ],
                'headlineSZ-2194' => [
                    'title' => 'Δραστηριότητες',
                    'attributes' => [
                        'maintitle' => 'Δραστηριότητες',
                        'maintitlesize' => '2.5rem'
                    ],
                    'block' => [
                        'id' => 'drastiriotites',
                        'variations' => 'nomarginall'
                    ]
                ],
                'imageSZ-3679' => [
                    'title' => 'η ζωή στο κατηχητικό',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_i_zoi_sto_katixitiko_1.jpg',
                        'link' => '0'
                    ]
                ],
                'imageSZ-1645' => [
                    'title' => 'δημιουργια',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_dimiourgia.jpg'
                    ]
                ],
                'imageSZ-6197' => [
                    'title' => 'συναγωνισμός',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_synagonismos.jpg',
                        'link' => '0'
                    ]
                ],
                'imageSZ-7475' => [
                    'title' => 'εξορμήσεις',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_exormiseis.jpg'
                    ]
                ],
                'imageSZ-8910' => [
                    'title' => 'γιορτές',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_giortes.jpg',
                        'link' => '0'
                    ]
                ],
                'imageSZ-7464' => [
                    'title' => 'mathainoume',
                    'attributes' => [
                        'image' => 'gantry-media://inaath_sto_ks_mathainoume.jpg'
                    ]
                ],
                'headlineSZ-9001' => [
                    'title' => 'Φωτογραφίες',
                    'attributes' => [
                        'maintitle' => 'Φωτογραφίες',
                        'maintitlesize' => '2.5rem',
                        'tag' => 'h2'
                    ],
                    'block' => [
                        'id' => 'fotografies',
                        'variations' => 'nomarginall'
                    ]
                ],
                'owlcarousel-2640' => [
                    'title' => 'Στιγμές δραστηριοτήτων',
                    'attributes' => [
                        'class' => 'drastiriotites-carousel',
                        'nav' => 'enable',
                        'dots' => 'enable',
                        'autoplay' => 'enable',
                        'autoplaySpeed' => '5000',
                        'imageOverlay' => 'disable',
                        'items' => [
                            0 => [
                                'name' => 'Photo 1',
                                'image' => 'gantry-media://Photo 1.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            1 => [
                                'name' => 'Photo 2',
                                'image' => 'gantry-media://Photo 2.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            2 => [
                                'name' => 'Photo 3',
                                'image' => 'gantry-media://Photo 3.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            3 => [
                                'name' => 'Photo 4',
                                'image' => 'gantry-media://Photo 4.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            4 => [
                                'name' => 'Photo 5',
                                'image' => 'gantry-media://Photo 5.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            5 => [
                                'name' => 'Photo 6',
                                'image' => 'gantry-media://Photo 6.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            6 => [
                                'name' => 'Photo 7',
                                'image' => 'gantry-media://Photo 7.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            7 => [
                                'name' => 'Photo 8',
                                'image' => 'gantry-media://Photo 8.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            8 => [
                                'name' => 'Photo 9',
                                'image' => 'gantry-media://Photo 9.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            9 => [
                                'name' => 'Photo 10',
                                'image' => 'gantry-media://Photo 10.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            10 => [
                                'name' => 'Photo 11',
                                'image' => 'gantry-media://Photo 11.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            11 => [
                                'name' => 'Photo 12',
                                'image' => 'gantry-media://Photo 12.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            12 => [
                                'name' => 'Photo 13',
                                'image' => 'gantry-media://Photo 13.jpg',
                                'title' => '',
                                'desc' => ''
                            ],
                            13 => [
                                'name' => 'Photo 14',
                                'image' => 'gantry-media://Photo 14.jpg',
                                'title' => '',
                                'desc' => ''
                            ]
                        ]
                    ]
                ],
                'customcontentSZ-7843' => [
                    'title' => 'Το κατηχητικό ξεκίνησε',
                    'attributes' => [
                        'maintitle' => 'Το κατηχητικό ξεκίνησε!',
                        'subtitle' => 'Σε περιμένουμε!',
                        'description' => '',
                        'textalign' => 'left'
                    ]
                ]
            ]
        ],
        'assignments' => [
            'page' => [
                0 => [
                    'home' => true
                ]
            ],
            'language' => [
                
            ],
            'type' => [
                
            ]
        ]
    ]
];
